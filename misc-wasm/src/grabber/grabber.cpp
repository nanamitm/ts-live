  ...
  if (frame) {
    ...
    if (frame->width > 0 && frame->height > 0 &&
      ...
      for (int i = 0, h2 = frame->height / 2; i < h2; ++i) {
        // 面ごとの先頭は data[] を使う
        // (buf[] は面ごとに分かれている保証がない)。
        // 色差の行を i ではなく i/2*2 にするのは、インタレ素材だと
        // 隣り合う色差の行が別のフィールドのものになり縞が出るため。
        const uint8_t *ly = frame->data[0] + frame->linesize[0] * i * 2;
        const uint8_t *lu = frame->data[1] + frame->linesize[1] * (i / 2 * 2);
        const uint8_t *lv = frame->data[2] + frame->linesize[2] * (i / 2 * 2);
